...
class SoloRails
  ...
  def self.search(*args)
    ...
    Rails.logger.info("Query URL: " + url)
    url
  ...
  private
    ...
    def self.parse_value(field_type, element)
      ...
      case field_type
        when 1, 4, 6, 11, 12 then element.text.to_s
        when 5 then { id: element.attribute("term").value, value: element.text.to_s }
    ...
    def self.parse_complex_date(element)
      ...
      begin
        ...
        ret = { :date => date, :display_date => "#{circa}#{nodate}#{ongoing}#{display_date}" }
      rescue
        ret = Chronic.parse("#{year}-06-01").strftime('%Y')
      end
      ret
    end
